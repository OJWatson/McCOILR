...
#'
#' This function triggers the c code for the proportional method, where we 
#' now store thinned chains and return these rather than writing to a file. 
#' 
...
#' @param saves After how many iterations do we save/write outputs. Default = 10
#' 
#' @return summary of output as data.frame
#' 

McCOIL_proportional_2018_R = function(dataA1, dataA2, maxCOI = 25, totalrun = 10000, burnin = 1000, 
                               M0 = 15, epsilon = 0.02, err_method = 1, path = getwd(), saves = 10) {
    ...
    paramList <- list(max = as.integer(maxCOI), iter = as.integer(totalrun), n0 = as.integer(n), 
                      ...
                      A = as.double(grid$A), B = as.double(grid$B), c0 = as.double(epsilon), 
                      err_method0 = as.integer(err_method), saves = saves)
    
    
    
    
    if ((n > 10 & k > 10)) {
        McCOIL_proportional_Rcpp(paramList = paramList)
    } else {
