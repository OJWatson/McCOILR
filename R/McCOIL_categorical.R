...
McCOIL_categorical = function(data, maxCOI=25, threshold_ind=20, threshold_site=20, 
                              totalrun=10000, burnin=1000, M0=15, e1=0.05, e2=0.05, 
                              err_method=1, path=getwd(), output="output.txt", thin=1){
    ...
    meanP= as.numeric(apply(outputMCMC1[burnin:totalrun, ((1:k)+n+1)], 2, mean))
    medianM= as.numeric(apply(outputMCMC1[burnin:totalrun, (1:n)+1], 2, median))
    medianP= as.numeric(apply(outputMCMC1[burnin:totalrun, ((1:k)+n+1)], 2, median))
    M975= as.numeric(apply(outputMCMC1[burnin:totalrun, (1:n)+1], 2, function(x) quantile(x, probs= 0.975)))
    P975= as.numeric(apply(outputMCMC1[burnin:totalrun, ((1:k)+n+1)], 2, function(x) quantile(x, probs= 0.975)))
    M025= as.numeric(apply(outputMCMC1[burnin:totalrun, (1:n)+1], 2, function(x) quantile(x, probs= 0.025)))
    P025= as.numeric(apply(outputMCMC1[burnin:totalrun, ((1:k)+n+1)], 2, function(x) quantile(x, probs= 0.025)))
    sdM= as.numeric(apply(outputMCMC1[burnin:totalrun, (1:n)+1], 2, sd))
    sdP= as.numeric(apply(outputMCMC1[burnin:totalrun, ((1:k)+n+1)], 2, sd))	
    
    if (err_method==3){
        mean_e1= as.numeric(mean(outputMCMC1[burnin:totalrun, (k+n+2)]))
        median_e1= as.numeric(median(outputMCMC1[burnin:totalrun, (k+n+2)]))
        e1_975=  as.numeric(quantile(outputMCMC1[burnin:totalrun,  (k+n+2)], probs= 0.975))
        e1_025=  as.numeric(quantile(outputMCMC1[burnin:totalrun,  (k+n+2)], probs= 0.025))
        sd_e1=  as.numeric(sd(outputMCMC1[burnin:totalrun, (k+n+2)]))
        
        mean_e2= as.numeric(mean(outputMCMC1[burnin:totalrun, (k+n+3)]))
        median_e2= as.numeric(median(outputMCMC1[burnin:totalrun, (k+n+3)]))
        e2_975=  as.numeric(quantile(outputMCMC1[burnin:totalrun,  (k+n+3)], probs= 0.975))
        e2_025=  as.numeric(quantile(outputMCMC1[burnin:totalrun,  (k+n+3)], probs= 0.025))
        sd_e2=  as.numeric(sd(outputMCMC1[burnin:totalrun, (k+n+3)]))	
    }
    if ((err_method==1) | (err_method==2)) {
        output_sum = data.frame(cbind.data.frame(rep(output, (n+k)),	
                                                ...
                                                c(meanM, meanP), 
                                                c(medianM, medianP), round(c(sdM, sdP),digits=5), 
                                                c(M025, P025), 
                                                c(M975, P975)))
    }
    else {
        output_sum= data.frame(cbind.data.frame(rep(output, (n+k+2)),	
                                                ...
                                                c(medianM, medianP, median_e1, median_e2), round(c(sdM, sdP, sd_e1, sd_e2),digits=5), 
                                                c(M025, P025, e1_025, e2_025), c(M975, P975, e1_975, e2_975)))
    }
